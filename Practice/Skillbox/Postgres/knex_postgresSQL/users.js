...
router.post("/", async (req, res) => {
    try {
        ...
        const [id] = await knex("users").insert(data).returning("id") //? повернемо id
        res.header(
            "Location",
            `${req.protocol}://${req.hostname}/api/users/${id}`
        ).sendStatus(201)
    } catch (error) {
        res.status(400).send(error.message)
    }
...
router.patch("/:id", async (req, res) => {
    ...
    try {
        ...
        if (updateCount === 0) {
            //? якщо відбулась помилка оновлення , поверне 0
            res.status(404).send(` Unknown user ID: ${id}`)
        } else {
            res.sendStatus(204)
        }
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        ...
        if (deleteCount === 0) {
            //? якщо відбулась помилка видалення , поверне 0
            res.status(404).send(` Unknown user ID: ${id}`)
        } else {
            res.sendStatus(204)
        }
